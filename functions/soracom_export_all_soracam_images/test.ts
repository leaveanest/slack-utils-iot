...
Deno.test("子 run が即完了した台をアップロードしたら次の待機台を補充する", async () => {
  ...
  try {
    ...
    assertEquals(cam6?.continuationTriggerId, "Ft6");
...
Deno.test("子 run がまだ processing の台は自分自身だけ再スケジュールする", async () => {
  ...
  assertEquals(cam1?.continuationTriggerId, "Ft6");
  assertEquals(cam6?.status, "queued");
  assertEquals(cam6?.continuationTriggerId, undefined);
  assertEquals(triggerDeletes, ["Ft1"]);
});
...
Deno.test("子 run が失敗した台は再試行待ちに戻して次の待機台を補充する", async () => {
  ...
  assertEquals(cam1?.errorMessage, undefined);
  assertEquals(cam6?.status, "queued");
  assertEquals(cam6?.continuationTriggerId, undefined);
  assertEquals(triggerDeletes, ["Ft1"]);
});
