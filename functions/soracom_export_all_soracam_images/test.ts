    ...
    workflows: {
      triggers: {
        ...
        delete(params: { trigger_id: string }) {
          ...
          const error = options?.triggerDeleteErrors?.[params.trigger_id];
          return Promise.resolve(
...
type RecordingBehavior =
  | {
    records: Array<{ startTime: number; endTime: number }>;
    events: [];
  }
  | Error;
...
function createSoracomClientMock(params: {
  ...
  initialExports?: Record<string, ExportBehaviorSequence>;
  resumedExports?: Record<string, ExportBehaviorSequence>;
  recordings?: Record<string, RecordingBehavior>;
  ...
  const soracomClient = {
    ...
    listSoraCamRecordingsAndEvents(deviceId: string) {
      ...
      const behavior = params.recordings?.[deviceId];
    ...
    exportSoraCamImage(deviceId: string, _time: number) {
      ...
      const behavior = nextInitialBehavior(deviceId);
      if (!behavior) {
...
Deno.test("進捗メッセージに失敗デバイスの詳細を含められる", async () => {
  await prepareLocale("ja");
  ...
  assertEquals(message.includes("失敗したデバイス"), true);
  assertEquals(message.includes("入口カメラ (cam-1): export_failed"), true);
  assertEquals(message.includes("倉庫カメラ (cam-2): HTTP 500"), true);
});
...
Deno.test("親 run は全ソラカメ一覧取得後に先頭バッチを起動する", async () => {
  await prepareLocale("ja");
  ...
  assertEquals(
    tasks.slice(0, ALL_SORACAM_EXPORT_PARALLELISM).map((task) =>
...
Deno.test("processing 継続用トリガーは retry delay を使って再スケジュールする", async () => {
  await prepareLocale("ja");

  const dateNowStub = stub(Date, "now", () => 1700000600000);
  ...
  try {
    ...
    assertEquals(
      triggerCreates.at(-1)?.schedule,
      {
        start_time: new Date(
          1700000600000 + ALL_SORACAM_EXPORT_TASK_RETRY_DELAY_MS,
        ).toISOString(),
        frequency: {
          type: "once",
        },
...
Deno.test("子 run が即完了した台をアップロードしたら次の待機台を補充する", async () => {
  ...
  try {
    const devices = createDevices(6);
    const { client, apiCalls, triggerCreates, triggerDeletes } =
      createExportAllClient();
    ...
    const cam2 = await getAllSoraCamImageExportTask(
      client as never,
      "C123:cam-2",
    );

    assertEquals(cam1?.status, "uploaded");
    assertEquals(cam1?.continuationTriggerId, undefined);
    assertEquals(cam2?.status, "processing");
    assertEquals(cam2?.continuationTriggerId, "Ft2");
...
Deno.test("子 run がまだ processing の台は自分自身だけ再スケジュールする", async () => {
  ...
  assertEquals(cam1?.continuationTriggerId, "Ft2");
  assertEquals(triggerDeletes, ["Ft1"]);
});
...
Deno.test(
  ...
  async () => {
    ...
    assertEquals(result.processingCount, ALL_SORACAM_EXPORT_PARALLELISM);
    assertEquals(result.failedCount, 0);
    assertEquals(tasks.map((task) => task.status), ["processing", "queued"]);
    assertEquals(tasks.map((task) => task.continuationTriggerId), [
      "Ft2",
      undefined,
    ]);
  },
...
Deno.test(
  "親 run は export_id 付きで止まった stale processing タスクを継続 trigger 付きで再開する",
  async () => {
    await prepareLocale("ja");
    ...
    if (!task) {
      throw new Error("task not found: C123:cam-1");
    }
    ...
    const resumedTask = await getAllSoraCamImageExportTask(
      client as never,
      "C123:cam-1",
    );
    ...
    assertEquals(triggerDeletes, ["Ft1"]);
    assertEquals(result.completedCount, 0);
    assertEquals(result.processingCount, 1);
    assertEquals(result.failedCount, 0);
    assertEquals(resumedTask?.status, "processing");
    assertEquals(resumedTask?.exportId, "exp-cam-1");
    assertEquals(resumedTask?.continuationTriggerId, "Ft2");
  },
);

Deno.test("子 run が失敗した台は再試行待ちに戻して次の待機台を補充する", async () => {
  await prepareLocale("ja");
  ...
  assertEquals(cam1?.status, "queued");
  assertEquals(cam1?.continuationTriggerId, undefined);
  assertEquals(cam1?.retryCount, 1);
  assertEquals(
    cam1?.errorMessage,
    "デバイス cam-1 のソラカメ画像スナップショット取得に失敗しました: failed",
  );
  assertEquals(cam2?.status, "processing");
  assertEquals(cam2?.continuationTriggerId, "Ft2");
  assertEquals(triggerDeletes, ["Ft1"]);
...
Deno.test("子 run は再試行上限到達後に failed と詳細を残す", async () => {
  await prepareLocale("ja");

  const devices = createDevices(1);
  const { client, updates, triggerCreates, triggerDeletes } =
    createExportAllClient();
  const { soracomClient, exportCalls } = createSoracomClientMock({
    devices,
    initialExports: {
      "cam-1": [
        buildExportResult("cam-1", "failed", "exp-cam-1-a"),
        buildExportResult("cam-1", "failed", "exp-cam-1-b"),
        buildExportResult("cam-1", "failed", "exp-cam-1-c"),
      ],
    },
  });
...
Deno.test("録画がない台は再試行せず failed にする", async () => {
  await prepareLocale("ja");

  const devices = createDevices(1);
  const { client, triggerCreates, triggerDeletes } = createExportAllClient();
  const { soracomClient, exportCalls, listRecordingCalls } =
...
Deno.test("trigger delete が trigger_not_found でも継続処理は完了できる", async () => {
  await prepareLocale("ja");

  const devices = createDevices(6);
  const { client, triggerDeletes } = createExportAllClient({
    triggerDeleteErrors: {
      Ft1: "trigger_not_found",
    },
  });
  const { soracomClient } = createSoracomClientMock({
    devices,
...
Deno.test("trigger delete の想定外エラーは warning に留めて処理を継続する", async () => {
  await prepareLocale("ja");
  ...
  try {
    ...
    assertEquals(warnStub.calls[0].args, [
